...
def get_feature_location_with_profile(
    ids, reference, profile_name, recordName, recordLocation, region
):
    """
    ...
    for record in query.all():
        ...
        for reg in all_reg:
            ...
            if fasta:
                ...
                fasta_output = utilities.createFASTAFromHMMOutput(
                    seq_record, hmmerout, record.name, reg_name
                )

                # Write the region sequence files out to a genome specific FASTA file
                fasta_outpath = "%s/%s%s.fasta" % (
                    outpath,
                    reg_name,
        ...
        if diagram:
            ToxinGraphicsMain.writeHMMToImage(
                hmmerout, outpath, seq_record, species, expand
            )
            print("Diagram has been written to %s directory" % (reference))

        if genbank:
            # TODO Add better invariant for Shotgun Naming
            ToxinGraphicsMain.writeHmmToSeq(
                hmmerout, outpath, seq_record, species, expand
            )
